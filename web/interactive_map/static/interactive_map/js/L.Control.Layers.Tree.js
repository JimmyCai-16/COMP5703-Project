    ...
    L.Control.Layers.Tree = L.Control.Layers.extend({
        ...
        _localExpand: function() {          
            ...
            if (this._map && L.DomUtil.hasClass(this._container, 'leaflet-control-layers-expanded')) {
                ...
                // Get all elements with class "leaflet-layerstree-closed"
                const closedElements = document.querySelectorAll('.leaflet-layerstree-closed');                
                if (closedElements.length > 0) {
                    // Filter out elements with class "leaflet-layerstree-hide"
                    const visibleClosedElements = Array.from(closedElements).filter(element => !element.classList.contains('leaflet-layerstree-hide'));
                    // Get the total count of visible closed elements
                    const totalCount = visibleClosedElements.length;
                    const button = document.querySelector('.leaflet-layerstree-toggleall');
                    if(totalCount == 0){                        
        ...
        _update: function() {
            ...
            this._addToggleAllButton();
            return this._localExpand();
        },

        _addToggleAllButton: function() {
            var container = this._container;
    
            // Create the toggle button
            var toggleAllButton = L.DomUtil.create('button', 'leaflet-layerstree-toggleall primary-blue-button', container);
            toggleAllButton.innerHTML = this.options.toggleAllLabel;

            var isExpanded = true;
    
            // Add a click event listener to toggle all layers when the button is clicked
            toggleAllButton.addEventListener('click', function() {
                isExpanded = !isExpanded;
                this._applyOnTree(container, isExpanded);
                toggleAllButton.classList.toggle('collapsed', !isExpanded);
                toggleAllButton.innerHTML = isExpanded ? 'Expand All' : 'Collapse All';
            }.bind(this));
    
            // Initialize the button state
            toggleAllButton.classList.toggle('collapsed', false);
        },
