...
- After running python manage.py migrate as required, a new data table named ‘porphyry_deposits_predictiondata’ will appear in the database, select it and run the SQL command:<br> 
   <h4>COPY porphyry_deposits_predictiondata(x, y, "SAMPLE_1", "SAMPLE_2", predicted_probabilities)
   FROM 'Location where you store your CSV file'
   DELIMITER ','
   CSV HEADER;</h4><br> 
  The csv file described above is called 'full_predicition.csv', it is the result set of all points generated by the prediction model, after importing the database, the system can query from the database to improve the query speed. The csv file will be delivered to the clients along with the other information.
- All .tif files need to follow the format of EPSG:4326, otherwise there will be consequences that the system will not run.
