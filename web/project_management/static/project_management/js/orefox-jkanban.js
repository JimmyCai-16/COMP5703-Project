  ...
  this.jKanban = function () {
    ...
    this.addElement = function (boardID, taskID, element, position) {
      ...
      if (typeof element.id !== 'undefined' && element.id !== '') {
        nodeItem.setAttribute('data-taskid', element.id)
      }
      ...
      for (var boardkey in boards) {
        ...
        headerBoard.innerHTML =
          '<div class="d-flex justify-content-between align-items-center w-100">'
            ...
            + '<div class="d-flex flex-row kanban-settings-dropdown d-none">'
              + '<div class="dropdown kanban-title-dropdown">'
                + '<div class="btn-clear-hover p-1" data-bs-toggle="dropdown">' 
                  + '<div data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="no-arrow-grey-tooltip" data-bs-title="Edit">'
                    + '<i class="fa-solid fa-pen fa-xs"></i>' 
                  + '</div>'
                + '</div>'
                + `<div class="dropdown-menu dropdown-menu-end mt-1 p-2 border border-1 shadow">`
                  + '<div class="d-flex flex-row justify-content-between">'
                  +   `<input name="title" data-title="${board.title}" class="no-border-input flex-grow-1">`
                  +   `<button class="btn btn-ofx-green btn-sm ms-2 d-flex flex-row align-items-center" type="submit">Done<i class="ms-2 fa-solid fa-turn-down fa-rotate-90"></i></button>`
                  + '</div>'
                + '</div>'
              + '</div>'
              + `<div class="btn-clear-hover p-1" data-bs-toggle="modal" data-bs-target="#deleteConfirmationModal" data-column-id="${board.id}">`
                +'<div data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="no-arrow-grey-tooltip" data-bs-title="Remove">'
        ...
        for (var itemkey in board.item) {
          ...
          nodeItem.classList.add('kanban-item')
          if (itemKanban.id) {
            nodeItem.dataset.taskid = itemKanban.id
          }
    ...
    this.getParentBoardID = function (el) {
      if (typeof el === 'string') {
        el = self.element.querySelector('[data-taskid="' + el + '"]')
      }
      if (el === null) {
        return null
      }
    ...
    this.replaceElement = function (el, element) {
      ...
      if (typeof nodeItem === 'string') {
        nodeItem = self.element.querySelector('[data-taskid="' + el + '"]')
      }
      nodeItem.innerHTML = __buildItemCard(element)
      // add function
      nodeItem.clickfn = element.click
    ...
    this.findElement = function (id) {
      var el = self.element.querySelector('[data-taskid="' + id + '"]')
      return el
    }
    ...
    function __buildItemCard(item) {
      ...
      let priority = 'priority' in item ? item.priority.toLowerCase() : '';
      let priorityCssColor = '';
      if (priority === 'h') {
        priority = 'High'
        priorityCssColor = 'var(--ofx-red)'
      } else if (priority === 'm') {
        priority = 'Medium'
        priorityCssColor = 'chocolate'
      } else {
        priority = 'Low'
        priorityCssColor = 'green'
      }
      let file = 'file' in item ? item.file : '';
      if (file.includes('/')) {
        // Usually file is saved with task/ prefix,  "task/file.html"
        file = file.split('/')[1]
      }
      ...
      const priorityHtmlDiv = priority !== '' ? `
      <div class="p-1 rounded" style="background-color: ${priorityCssColor}; color: white; font-size: small"
        data-bs-toggle="tooltip" data-bs-placement="left"
        data-bs-custom-class="no-arrow-grey-tooltip"
        data-bs-title="Priority"
      >
        ${priority}
      </div>
      ` : '';

      const assigneeHtmlDiv = assigneesLength > 0 ? `
        <div class="ms-2" style="color: grey; font-size: small"
          data-bs-toggle="tooltip" data-bs-placement="bottom"
          data-bs-custom-class="task-card-tooltip"
          data-bs-title="${assignees.join(', ')}"
        >
          ${userIcon}
          ${assigneesLength}
        </div>
      ...
      let content = 
        '<div style="cursor: pointer;" class="text-wrap">' 
          +`<div class=" text-wrap">
          ...
          +`<div
              ...
              data-bs-custom-class="task-card-tooltip"
              data-bs-title="${taskDueDateToolTip(date)}"
            >
              <p class="fw-light" style="color:grey; font-size: small">
              ${calendarIcon} ${date && formatTaskDueDate(date)} 
              </p>
            </div>`
